...
public class TurtleVsCrocodileGame
{
  ...
  /// カメの現在位置
  /// </summary>
  private static int _kamePos = 0;

  /// <summary>
  /// ワニの現在位置
  /// </summary>
  private static int _waniPos = 10;

  /// <summary>
  /// カメとワニの距離
  /// 注意: この値は毎ループの最初に計算される必要がある
  /// </summary>
  private static int _kameWaniDistance = 0;
  ...
  public void Run()
  {
    ...
    while (i < 10)
    {
      ...
      _kameWaniDistance = Math.Abs(_kamePos - _waniPos);

      // -----------------------------------------------------------------
      // ステップ2-3: キャラクターの移動処理
      // -----------------------------------------------------------------
      // 注意: moveKame() は kameWaniDistance の値に依存するため、
  ...
  /// <remarks>
  /// 問題点:
  /// - グローバル変数 _kameWaniDistance を直接参照している
  /// - グローバル変数 _kamePos を直接変更している
  /// - このメソッドが呼ばれる前に _kameWaniDistance が
  ///   正しく計算されていることを前提としている（暗黙の契約）
  /// </remarks>
  private static void moveKame()
  {
    // ワニとの距離が2より大きい場合は安全と判断して前進
    if (_kameWaniDistance > 2)
    {
      _kamePos++;
      Console.WriteLine("カメは歩く");
    }
    // ワニが近い場合は防御態勢
    else if (_kameWaniDistance <= 2)
    {
